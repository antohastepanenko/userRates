...
public sealed class GetUserCurrencyRatesQueryHandlerTests
{
    ...
    {
        var favorites = new StubFavoritesClient(Result<IReadOnlyList<string>>.Ok(new[] { "USD" }));
    ...
    public async Task Returns_only_available_rates_for_the_current_users_favorites()
    {
        var favorites = new StubFavoritesClient(
            Result<IReadOnlyList<string>>.Ok(new[] { "usd", "EUR", "USD", "JPY" }));
        ...
        result.Value.Items.Select(item => item.Rate).Should().Equal(100.2m, 90.12567891m);
    ...
    {
        var favorites = new StubFavoritesClient(
            Result<IReadOnlyList<string>>.Ok(Array.Empty<string>()));
        var rates = new StubCurrencyRateReader(Array.Empty<Currency>());
        var handler = new GetUserCurrencyRatesQueryHandler(
            new StubCurrentUser(isAuthenticated: true, userId: UserId), favorites, rates);
    ...
    {
        private readonly Result<IReadOnlyList<string>> _result;

        public StubFavoritesClient(Result<IReadOnlyList<string>> result)
        {
            _result = result;
        }
        ...
        public Task<Result<IReadOnlyList<string>>> GetFavoriteCodesAsync(
            Guid userId,
